...
enum CollisionOperatorType {
    BGK,
    TRT,
    MRT
};

template<int dim, enum CollisionOperatorType coll_op>
struct Params;

template<>
struct Params<2, CollisionOperatorType::BGK>  {
...
template<>
struct Params<2, CollisionOperatorType::TRT> {
...
public:
    static constexpr CollisionOperatorType TYPE = CollisionOperatorType::BGK;

    BGKCollisionStrategy(const double _init_vel, const Params<dim, BGK>& params) :
...
template<int dim, class VelocitySet>
class TRTCollisionStrategy {
public:
    static constexpr CollisionOperatorType TYPE = CollisionOperatorType::TRT;
