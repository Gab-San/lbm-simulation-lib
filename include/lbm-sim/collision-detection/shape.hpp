...
class Airfoil : public Shape<dim, Airfoil<dim>> {
  ...
  mutable std::vector<types::Coordinate<dim>> cached_perimeter;
  mutable std::vector<std::pair<double, double>> cached_polygon; // punti normalizzati (x,y) del contorno chiuso

  // genera il contorno chiuso (upper poi lower al contrario) in coordinate normalizzate [0,1] di corda
  void buildPolygon() const {
    if (!cached_polygon.empty()) return;
    ...
    for (int i = 0; i <= N; ++i) {
      ...
      // definisco la costante M_PI per sicurezza, anche se dovrebbe essere definita in cmath
      ...
      if (max_camber > 0.0 && camber_pos > 0.0) {
        if (x < camber_pos) {
          yc = (max_camber / (camber_pos*camber_pos)) *
               (2*camber_pos*x - x*x);
          dyc_dx = (2*max_camber / (camber_pos*camber_pos)) *
                   (camber_pos - x);
        } else {
    ...
    // contorno chiuso: upper da leading a trailing, poi lower da trailing a leading
  ...
  types::Coordinate<dim> toGrid(double xn, double yn) const {
    ...
    return position + types::Coordinate<dim>(
...
public:
  ...
  std::vector<types::Coordinate<dim>> &getPerimeter() const {
    if (!cached_perimeter.empty()) return cached_perimeter;
    buildPolygon();
    ...
    for (int dx = -half; dx <= half; ++dx) {
      for (int dy = -half; dy <= half; ++dy) {
        const types::Coordinate<dim> p = position + types::Coordinate<dim>(dx, dy);
        if (contains(p)) {
          cached_perimeter.push_back(p);
        }
      }
