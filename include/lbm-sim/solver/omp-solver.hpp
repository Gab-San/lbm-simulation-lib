    ...
    for (auto y = 0; y < lattice.grid.size.y; ++y) {
      for (auto x = 0; x < lattice.grid.size.x; ++x) {
        ...
        for (auto diridx = 0; diridx < D2Q9::ndir; ++diridx) {
          // for each neighbor src we need to compute the bounce back 
          // on its specific direction, if it is a boundary node!!!
          const types::Coordinate<2> src = p - D2Q9::dir[diridx];

          // if source node is external it is on a boundary node
          // BUT THIS ALSO HAS TO APPLY TO INTERNAL OBJECTS, WHICH ARE ALSO BOUNDARY NODES!!!
          if (!lattice.grid.contains(src) || lattice.boundary_mask[lattice.grid.scalar_index(src)] == Solid::BB_INNER) {
            // if source node is external it is on a boundary node
