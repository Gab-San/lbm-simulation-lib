...
//
// /// This class stores the values to use, and allows them to be applied to the
// /// runtime.
...
//   }
//
//   //   /// RAII counterpart of `apply()`: snapshots the runtime's ICVs,
//   applies
//   //   the
//   //   /// stored properties, and restores the snapshot on scope exit. Use it
//   to
//   //   /// keep a profiled region from leaking its settings into whatever
//   runs
//   //   /// next:
//   //   ///
//   //   ///     {
...
//   //   ///
//   //   /// Same threading rule as `apply()`: construct it on the initial
//   thread,
//   //   /// outside any parallel region.
//   //   class [[nodiscard]] Scope {
//   //   public:
//   //     explicit Scope(const BackendProperties &props) noexcept
//   //         // omp_get_max_threads() reports the *nthreads-var* ICV, i.e.
//   //         exactly
//   //         // what omp_set_num_threads() last set, not a thread count in
//   //         flight. : prev_num_threads(omp_get_max_threads()),
...
public:
  ...
  unsigned int getNumThreads() const noexcept { return num_threads; }
  omp_sched_t getSchedule() const noexcept { return schedule; }
  int getChunkSize() const noexcept { return chunk_size; }
  bool getDynamicThreads() const noexcept { return dynamic_threads; }
  ...
  /// `chunk_size_` <= 0 leaves the chunk size to the runtime.
  void setSchedule(const omp_sched_t schedule_,
                   const int chunk_size_ = 0) noexcept {
    schedule = schedule_;
    chunk_size = chunk_size_;
  }
...
private:
  ...
  BackendProperties() noexcept
      : num_threads(static_cast<unsigned int>(omp_get_max_threads())),
        schedule(omp_sched_static), chunk_size(0), dynamic_threads(false) {}
