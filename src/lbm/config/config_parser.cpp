#include "lbm/config/config_parser.hpp"
...
CollisionOperator parse_op(std::string s) {
  ...
  throw std::runtime_error(
      "[Error while parsing file] Operator not recognised");
}
  ...
  while (std::getline(in, line)) {
    ...
    while (parsing_state != -1) {
      ...
      case 3:
        if (toks[0] != "U_lid")
          throw std::runtime_error(
              "[Error while parsing] U_lid line expected!");
      ...
      case 4:
        if (toks[0] != "Nsteps")
          throw std::runtime_error(
              "[Error while parsing] Nsteps line expected!");
      ...
      case 5:
        if (toks[0] != "Nframes")
          throw std::runtime_error(
              "[Error while parsing] Nframes line expected!");
      ...
      case 6:
        if (toks[0] != "out_norm")
          throw std::runtime_error(
              "[Error while parsing] out_norm line expected!");
      ...
      case 7:
        if (toks[0] != "out_bench")
          throw std::runtime_error(
              "[Error while parsing] out_bench line expected!");
      ...
      case 8:
        if (toks[0] != "Col")
          throw std::runtime_error(
              "[Error while parsing] collision line expected!");
      ...
      default:
        throw std::runtime_error(
            "[Error while parsing] Reached an undefined parsing state!");
      }
