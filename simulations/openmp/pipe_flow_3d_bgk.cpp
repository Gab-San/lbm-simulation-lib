...
int main(int argc, char **argv) {
  ...
  for (const auto &cfg : configs) {
    ...
    LBM_LOG_INFO(
        ...
        "output: {}\n\tProfile output: {}",
        cfg.name, grid_size, cy, cz, radius, cfg.reynolds, u0, cfg.niters,
    ...
    LBMSimulation<DIM, D3Q19, COLLISION> simulation(
        grid_size, std::move(solid_mask), obstacle_data, dbc, params, pin,
        pout);
    ...
    const auto exact_solution =
        analysis::HagenPoiseuilleSolution3D(R_eff, u0.dx, cy, cz);
    const double err_l2 =
