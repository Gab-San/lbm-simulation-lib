...
/// TRT le prende lid_cavity_d2q9_trt.
...
int main(int argc, char **argv) {
  ...
  // Prima di qualunque altra cosa: se la config e' sbagliata non ha senso
  // tirare su il logger ne' allocare la griglia.
  if (argc != 4) {
    print_usage(argv[0]);
    return 1;
  }
  ...
  try {
    ...
    // Il logger non c'e' ancora: questi errori vanno su stderr.
  ...
  // pareti rigide + il lid mobile in alto. Ricavati da nx/ny.
  ...
  const std::vector<CollisionDetection::CollisionArea<DIM>> obstacles{
      CollisionDetection::CollisionArea(A,
                                        {CollisionDetection::Segment(A, B),
